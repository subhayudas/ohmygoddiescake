...
> **This is the authoritative context for the whole codebase.** Codex automatically
...
> 1. Codex loads `AGENTS.md` into context on every session start — no manual step needed.
> 2. A `PostToolUse` hook in `.Codex/settings.json` fires after any `Edit`/`Write` to a
>    source file and reminds Codex to reconcile the affected section of this file before
...
Dev server config lives in `.Codex/launch.json` (port 3000).
...
ohmygoddiescake_work/
  ...
  components/
    ...
    Reviews.tsx           # Testimonials + seamless 9-image portfolio marquee (`#gallery`)
  ...
  public/
    ...
    WhatsApp Image*.jpeg  # Three recent wedding-cake photos used in the Reviews gallery
  ...
  .Codex/
    ...
    settings.json       # Codex hooks (incl. AGENTS.md sync reminder)
  ...
  SESSION_*.md                # Transient Codex session logs — not project docs, safe to ignore
```
...
This file is the codebase context every Codex session starts from, so it must stay accurate.

- **Auto-loaded:** Codex reads `AGENTS.md` into context automatically at session start.
  New sessions get the full picture here instead of re-reading every source file.
- **Auto-reminded:** `.Codex/settings.json` defines a `PostToolUse` hook (matcher `Edit|Write`)
  that fires whenever a source file under `app/`, `components/`, `lib/`, `scripts/`, or a root
  config file is edited. The hook injects a reminder for Codex to update the relevant section of
...
**2026-08-11** — Added three new wedding-cake photos from `public/WhatsApp Image*.jpeg` to the
portfolio grid in `Reviews.tsx`, expanded the gallery to a balanced 3-column / 9-image layout, and
added descriptive alternative text for the new images.
...
**2026-06-14** — Synced AGENTS.md to the live codebase: corrected page composition (Pricing not
